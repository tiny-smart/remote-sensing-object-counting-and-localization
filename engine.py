...
def evaluate(model, data_loader, device, epoch=0, criterion=None,vis_dir=None):
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        Precision_s = tp / (tp + fp + 1e-16)
        Recall_s = tp / (tp + fn + 1e-12)
        F1_s = 2 * (Precision_s * Recall_s) / (Precision_s + Recall_s + 1e-8)

        # Converting counts to lists for r2_score
        # r2 = r2_score([gt_cnt], [predict_cnt])
        
        # 检查 gt_cnt 是否有非零方差
        if np.var([gt_cnt]) == 0:
            # 如果方差为零，可以返回0或设定一个默认值
            r2 = 0
        else:
            r2 = r2_score([gt_cnt], [predict_cnt])
        # if np.isnan(r2):
        ...
        results['mae'], results['mse'], results['Precision_s'], results['Recall_s'], results['F1_s'], results['abs'], \
        results['r2'] = toTensor(mae), toTensor(mse), Precision_s, Recall_s, F1_s, abs_, r2
        
        results['r2'] = toTensor(r2)

        metric_logger.update(mae=results['mae'], mse=results['mse'], Precision_s=results['Precision_s'],
                             Recall_s=results['Recall_s'], F1_s=results['F1_s'], abs=results['abs'], r2=results['r2'])

        results_reduced = utils.reduce_dict(results)
        metric_logger.update(mae=results_reduced['mae'], mse=results_reduced['mse'], Precision_s=results['Precision_s'],
                             Recall_s=results['Recall_s'], F1_s=results['F1_s'], abs=results['abs'], r2=results_reduced['r2'])
