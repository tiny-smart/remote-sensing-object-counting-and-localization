...
def get_args_parser():
    ...
    parser.add_argument('--pretrained', default='../store/pretrained/vgg16_bn-6c64b313.pth', type=str,
...
def print_model_parameters(model, description):
    print(description)
...
def test_main(args, resume):
    ...
    mae, mse, F1, abs = test_stats['mae'], test_stats['mse'], test_stats['F1_s'], test_stats['abs']
    return mae, mse, F1, abs
...
def main(args):
    ...
    # print_model_parameters(model_without_ddp, "Initial model parameters:")
    # load pretrained model
    if args.resume:
        ...
        model_without_ddp.load_state_dict(checkpoint['model'], False)
        # print_model_parameters(model_without_ddp, "Model parameters after loading checkpoint:")
        if 'epoch' in checkpoint:
            cur_epoch = checkpoint['epoch']
    ...
    mae, mse, P, R, F, abs, r2 = test_stats['mae'], test_stats['mse'], test_stats['Precision_s'], test_stats['Recall_s'], \
    test_stats['F1_s'], test_stats['abs'], test_stats['r2']
    line = f'\nepoch: {cur_epoch}, mae: {mae}, MSE: {mse}, Precision: {P},Recall: {R}, F1: {F},abs: {abs}, r2:{r2}'
    print(line)
...
def test(args, resume):
    mae, mse, F1, abs = test_main(args, resume)
    return mae, mse, F1, abs