...
class EnWinDecoderTransformer(nn.Module):
    ...
    def __init__(self, d_model=256, nhead=8, num_decoder_layers=2,
        ...
        self.endecoder = TransformerEnDecoder(decoder_layer, encoder_layer, num_decoder_layers, decoder_norm,
                                              return_intermediate=return_intermediate_dec)
        self._reset_parameters()
    ...
    def decoder_forward(self, query_feats, query_embed, memory_win, pos_embed_win, mask_win, dec_win_h, dec_win_w,
        ...
        hs_win = self.endecoder(tgt, memory_win, memory_key_padding_mask=mask_win, pos=pos_embed_win,
                                query_pos=query_embed_win, **kwargs)
...
class TransformerEnDecoder(nn.Module):
    ...
    def __init__(self, decoder_layer, encoder_layer, num_layers, norm=None, return_intermediate=False):
        super().__init__()
        self.enlayers = _get_clones(encoder_layer, num_layers)
        self.layers = _get_clones(decoder_layer, num_layers)
        self.num_layers = num_layers
        self.norm = norm
        self.return_intermediate = return_intermediate
    ...
    def forward(self, tgt, memory,
        ...
        for i in range(self.num_layers):
            layer = self.layers[i]
            enlayer = self.enlayers[i]
            output = enlayer(output, pos=query_pos)
            output = layer(output, memory, tgt_mask=tgt_mask,
                           memory_mask=memory_mask,
                           tgt_key_padding_mask=tgt_key_padding_mask,
                           memory_key_padding_mask=memory_key_padding_mask,
