...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        if self.shift_size > 0:
            ...
            attn_mask = attn_mask.masked_fill(attn_mask != 0, float(-10000000.0)).masked_fill(attn_mask == 0, float(0.0))
        else:
            attn_mask = None
    ...
    def forward_features(self, x):
        ...
        if self.multi_scale:
            ...
            for i, layer in enumerate(self.layers):
                x = layer(x)
                x_2d = x.view(B, H // int(8 * self.scales[i]), W // int(8 * self.scales[i]), -1).permute(0, 3, 1,
                                                                                                   2)  # B C H W
